  ...
  it("uploads an event-delivered token with the exact push_tokens row shape", async () => {
    ...
    await vi.waitFor(() =>
      expect(mocks.registerPushToken).toHaveBeenCalledWith({
        ...
        // Round-18 audit fix: a fresh crypto.randomUUID() per registration attempt,
        // the compare-and-swap key unregisterPushToken's DELETE conditions on.
...
describe("usePushRegistration — cleans up a stale token when the gate no longer holds (round-14/15 audit fix)", () => {
  it("unregisters the token when a Pro user's license downgrades to free (dep change, stays mounted)", async () => {
    ...
    // No token was ever delivered this session, so registeredForRef is still null — the
    // cold-start fallback path, no nonce to compare-and-swap against (round-18 audit fix).
    await vi.waitFor(() => expect(mocks.unregisterPushToken).toHaveBeenCalledWith("user-1", "device-1", undefined, expect.any(String)));
  });
  ...
  it("unregisters the token when the whole hook unmounts after a successful registration — the real InterruptHandler Pro-gate shape, not just a dep change while mounted", async () => {
    ...
    // A real registration happened this session, so the ref (and thus the round-18
    // compare-and-swap nonce) is populated — the exact value is a crypto.randomUUID()
    // generated by production code, not asserted literally.
    await vi.waitFor(() => expect(mocks.unregisterPushToken).toHaveBeenCalledWith("user-1", "device-1", expect.any(String), undefined));
  ...
  // Round-18 audit fix (Agent B, "ghost registration" finding): round 17's narrower
  // trulyUnmountedRef check ONLY self-cleaned on a true unmount — an in-flight
  // registration superseded by an ORDINARY dep change (not an unmount) was left to
  // write registeredForRef unconditionally, orphaning a "ghost" row with no future
  // cleanup opportunity if the gate had also flipped false in the meantime. Round 18
  // broadened the check to `cancelled` (set by this effect run's own cleanup on EITHER
  // an unmount OR a dep-change re-run), so this attempt now self-cleans here too. This
  // is safe specifically because it's scoped to this attempt's OWN nonce (CAS-protected)
  // — it can only ever delete a row that still holds that exact value, so it can never
  // touch whatever a newer effect run's own fresh registration writes, regardless of
  // resolution order. Deletion Test: reverting to the narrower trulyUnmountedRef-only
  // check makes this test fail — no unregisterPushToken call at all.
  it("self-cleans an in-flight registration that resolves after an ordinary dep change (not just a true unmount) — safe because it's scoped to this attempt's own nonce", async () => {
    let resolveRegister!: (v: { ok: true }) => void;
    mocks.registerPushToken.mockImplementationOnce(
      () => new Promise((resolve) => { resolveRegister = resolve; }),
    );
    ...
    state.tokenHandler!("a1b2c3");
    await vi.waitFor(() => expect(mocks.registerPushToken).toHaveBeenCalledTimes(1));
    const firstCall = mocks.registerPushToken.mock.calls[0] as unknown as [{ registrationNonce: string }];
    const firstNonce = firstCall[0].registrationNonce;
    ...
    // Self-cleans using exactly the nonce THIS (now-superseded) attempt generated, not
    // an unconditional delete — proving the CAS protection carries through correctly.
    await vi.waitFor(() => expect(mocks.unregisterPushToken).toHaveBeenCalledWith("user-1", "device-1", firstNonce, undefined));
  });
  ...
  // Round-18 audit fix, core design assumption: registration_nonce must be a fresh value
  // per registration ATTEMPT, independent of the OS-issued token, because the token is
  // stable per install and commonly repeats across attempts on the same device (a fast
  // Deactivate-then-Reactivate typically redelivers the SAME cached token) — a token-value
  // compare-and-swap would not have distinguished the old, superseded registration from the
  // new one. Deletion Test: replacing `crypto.randomUUID()` with a value derived from the
  // token (e.g. the token itself) makes this test fail, since both calls deliver the
  // identical token "same-token".
  it("generates a fresh registration nonce per attempt even when the delivered token is identical both times", async () => {
    const { rerender } = renderHook(() => usePushRegistration());
    await vi.waitFor(() => expect(typeof state.tokenHandler).toBe("function"));
    state.tokenHandler!("same-token");
    await vi.waitFor(() => expect(mocks.registerPushToken).toHaveBeenCalledTimes(1));
    const firstCall = mocks.registerPushToken.mock.calls[0] as unknown as [{ registrationNonce: string }];
    const firstNonce = firstCall[0].registrationNonce;

    // A second registration attempt (dep change, gate stays true) delivering the SAME token.
    state.entitlement.validUntil = Date.now() + 86_400_000;
    rerender();
    await vi.waitFor(() => expect(typeof state.tokenHandler).toBe("function"));
    state.tokenHandler!("same-token");
    await vi.waitFor(() => expect(mocks.registerPushToken).toHaveBeenCalledTimes(2));
    const secondCall = mocks.registerPushToken.mock.calls[1] as unknown as [{ registrationNonce: string }];
    const secondNonce = secondCall[0].registrationNonce;
