...
**All four levels (A1, A2, B1, B2) are now complete on unit count, density, AND have been through at least one adversarial content-quality audit + remediation round.** A1/A2's original 2026-07-30 audit (61/100 pedagogical rigor, 74/100 brand/voice) was followed only by mechanical backfills (empty hints, tier-1-context, tier-4-passage structure) for a long stretch — the adversarial prose-rewrite remediation B1 and B2 each received was not run on A1/A2 until 2026-08-03's round-2 audit (68/100 rigor, 77/100 avg brand/voice) and matching remediation. See the dated notes below for the full history of each level's audit rounds and what each one fixed.

| Level | Units | Cards | Target cards | Status |
|-------|-------|-------|--------------|--------|
| A1 | 20/20 | 2,810 | ~2,600 | COMPLETE (2026-08-03) — card count at/above target; round-2 content-quality audit + remediation done (pedagogical rigor 68/100, brand/voice 78/100 — up from the 2026-07-30 baseline of 61/74) |
| A2 | 30/30 | 5,392 | ~5,700 | COMPLETE (2026-08-03) — card count near target; round-2 content-quality audit + remediation done (pedagogical rigor 68/100, brand/voice 76/100 — up from the 2026-07-30 baseline of 61/74) |
...
| B2 | 40/40 | 14,892 | ~15,200 | COMPLETE (2026-08-03) — unit count, density, and a 3-round content-quality audit/remediation arc all done (pedagogical rigor avg 69→final round targeted every confirmed finding, brand/voice avg 79/100) |
| **Total** | **126/125** | **30,609** | **~32,950** | Unit count exceeds the original 125-unit plan (B1 and B2 each ended up with one extra unit past their original targets — the B1 imperative unit and B2's reading-passages capstone); ~93% of the way to the full target by card count. All four levels now have unit-count/density completion AND at least one adversarial content-quality audit + remediation round. |
...
**2026-08-03 — B2 content-quality audit, round 2 (re-audit).** Same 16-file sample, same two-reviewer methodology, checking round 1's fixes against actual current card content rather than trusting commit messages. Scores were mixed by reviewer: one scored **83/86** (up sharply from 63/74), the other scored **68/79** (barely moved from 70/82) — a real discrepancy, not just reviewer noise, because the second reviewer's low score was driven by specific, mechanically-verifiable findings the first reviewer's sample didn't surface. Both reviewers' specific claims were independently checked against the actual file content (not just averaged and trusted): `u29-t2-022`'s hint literally stated the accepted answer (`"tu form is identical to io: avessi"` for accepted `"avessi"`), `u42-t3-003`'s hint was just the collocation with the blanked word spelled out (`"nell'eventualità che + congiuntivo"` for a blank testing `"eventualità"`), and `u37-t3-050` still had the exact "fare i conti senza l'oste" mistranslation a round-1 fix was supposed to have eliminated (fixed at the tier-1 definition, not at this tier-3 card). All three confirmed real on inspection.

**Round 3 remediation**, targeting only the confirmed-real round-2 findings (not every round-2 claim was pursued at equal depth — some, like `b2-unit-48`'s partially-cosmetic hint fixes, were judged low enough severity to leave as-is): answer-leaking hints fixed at scale in `b2-unit-29` (74 of 144 tier-2 hints, independently recounted and matched exactly) and `b2-unit-42` (28 of 31 tier-3 hints); `b2-unit-29`'s remaining 24 bare conjugation-table cards (beyond the range round 1's fix agent had scoped to) rewritten into real sentences; a genuine Italian grammar error confirmed and fixed — `recedere` in the contractual-withdrawal sense conjugates with **avere**, not essere, contra 4 cards' `essere`-based gender-agreeing forms (verified against Treccani, not just asserted); `b2-unit-37`'s true orphaned-content count turned out to be 33 items (27 false-friend words + 6 proverbs), not the round-2 estimate of ~30 — all 33 given real live replacement cards; `b2-unit-45`'s remaining ~14 word-pairs of bare gender-only hints fixed (one more pair found beyond the named list during a full read-through); a dozen duplicate-tag cards and one more grammar-terminology mislabeling fixed in `b2-unit-36`; 3 residual doubled-gratitude letter closings fixed in `b2-unit-68`; one residual "moral of the story" passage ending fixed in `b2-unit-58`.

This round explicitly instructed every fix agent never to run `git stash`, closing the exact gap that caused round 1's data-loss incident — all 7 round-3 fix agents completed with no reversion, confirmed by re-checking every specific fix marker directly against the working tree afterward rather than trusting the reports.

**Final verification for this remediation arc:** `npx tsc --noEmit` clean, `pack:export`/`pack:validate`/`pack:lint-quality` all clean (hard gates pass, zero new baseline violations), `scripts/checkCardIds.ts` against the pre-remediation commit confirms zero cards removed anywhere (+219 net new cards), full test suite (1503/1503), lint clean, and `public/packs/manifest.json`'s `es` (Spanish) pack entry — which one fix agent discovered `exportPack.ts` can silently drop on some re-export runs, logged as a separate tooling bug, not yet investigated — confirmed intact after the final export.

A fourth audit round was not run. Given two of three round-2 reviews already showed strong, verified improvement and the third round's fixes targeted every finding that review's own reviewer scored as real and severe, this is a reasonable point to stop and let Max judge whether further rounds are worth it — the same diminishing-returns judgment call this file already documents making for B1.

**2026-08-03 — A1/A2 content-quality audit round 2 (the first adversarial prose remediation either level has received).** Same 2-reviewer methodology as B1/B2, on a fresh 10-unit sample (5 A1, 5 A2) chosen to cross-check specific things: `a1-unit-08-review` (the modal-verb unit backfilled 2026-07-30, to check the backfill's own quality), `a1-unit-11-food` (CURRICULUM.md's own cited model/reference unit, to confirm that reputation holds), `a2-unit-16-conditional` (the unit this file already flags as having one pre-existing, deliberately-deferred congiuntivo leak), `a2-unit-38-time-expressions` (another 2026-07-30 backfill), plus 6 more units spanning early/late A1 and general A2 vocabulary domains. Scored **pedagogical rigor 68/100** (up from the 2026-07-30 baseline of 61) and **brand/voice fidelity 77/100 average** (78 A1-half / 76 A2-half, up from 74) — confirming the mechanical 2026-07-30 backfills (hints, tier completeness, passage structure) genuinely held up under adversarial inspection, but that no prose-level rewrite pass had ever touched A1/A2 the way B1 and B2 each received.

Real findings, several worse than this file's own prior tracking believed: the previously-deferred `a2-unit-16` congiuntivo leak ("se potessi," `u16-t1-010`) turned out to have **3 more untracked subjunctive instances** spread across tiers 1/3/4 of the same file — the unit's actual subjunctive footprint was 4× what CURRICULUM.md's audit trail recorded. A second, entirely new subjunctive leak point surfaced in `a2-unit-30-technology-internet` (fixed triggers `prima che`/`aspettare che` grammatically requiring congiuntivo — 3 instances once the fix agent's own corpus grep found one more beyond the two named). `a1-unit-08-review`'s tier-3 section had the exact "bare modal+infinitive, conjugation drill wearing collocation costume" pattern CURRICULUM.md documents fixing in B1 (2026-08-02) — confirmed still present in A1 and never addressed (13 of 28 cards rewritten). The 2026-07-30 tier-1-context backfill itself left a visible seam across multiple A1/A2 units: sentences built to satisfy the mechanical gate by stacking 2-3 unrelated vocabulary words into one unnatural clause rather than teaching real usage — found and fixed in `a1-unit-01`, `a2-unit-16`, `a2-unit-21`, and `a2-unit-39` (the latter's fix agent found 2 more instances of the pattern beyond the 3 originally named, all resolved by either simplifying to a natural single-topic sentence or splitting into two genuine sentences). Also fixed: a missed near-duplicate in `a1-unit-01` (three sibling duplicates had already been correctly retired via `deprecated: true`, a fourth wasn't), an asymmetric tier-1-context fix within one gender pair (`l'italiano` never got the context its feminine sibling `l'italiana` received), a translation-fidelity slip in `a2-unit-21` (the English prompt's "always" was silently dropped from every accepted Italian answer), and 5 exclamation-mark voice-tone slips in hints across `a1-unit-08`/`a1-unit-11` (BRAND.md's quiet-expert voice) — the one defect class this project's own mechanical gate documentation says it structurally cannot catch.

All 7 fix agents this round were explicitly instructed never to run `git stash`, per the lesson from B2's round-1 data-loss near-miss — all 7 completed with zero reversion, independently confirmed by re-checking each specific fix marker directly against the working tree afterward.

**Final verification:** `npx tsc --noEmit` clean, `pack:export`/`pack:validate`/`pack:lint-quality` all clean (hard gates pass, zero new baseline violations), `scripts/checkCardIds.ts` against the pre-remediation baseline confirms zero cards removed anywhere (+224 net new), full test suite (1503/1503), lint clean, and the `manifest.json` `es` entry (protected by the atomic-write fix from earlier the same day) confirmed intact. A third round was not run for A1/A2 either — same diminishing-returns judgment left open for Max.
...
**BASELINE (ratchet) gates** — real, large existing debt is grandfathered as of the 2026-07-30 baseline capture (`scripts/cardQualityBaseline.it.json`); any violation NOT in the baseline (i.e. introduced by a new or edited card) fails the gate. The baseline must only ever shrink — same convention as this project's coverage thresholds (AGENTS.md: "thresholds only ever increase — ratchet up, never down"). After a content pass fixes baselined debt, run `npx tsx scripts/lintCardQuality.ts public/packs/it.json scripts/cardQualityBaseline.it.json --update-baseline` to shrink the file to match. **Baselines are per-language** (`scripts/cardQualityBaseline.{lang}.json` — the file was renamed from the unqualified `cardQualityBaseline.json` when multi-language support was added, since Italian's grandfathered debt must never be checked against a different language's pack, and vice versa):
