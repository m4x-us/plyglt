...
- **Fix applied:** added `"createUpdaterArtifacts": true` to `src-tauri/tauri.conf.json`'s `bundle` section (not `"v1Compatible"` — this app was never on Tauri v1, so the modern `true` value is correct per the docs). Verified: valid JSON, `cargo check` clean, `npx tauri info` parses the config without error, full app verification gate (tsc, 1510/1510 tests) unaffected — this is a bundler-only config addition, doesn't touch application code.
- **NOT verified end-to-end** — this task's own "Done when" criteria (a real tagged release producing `latest.json` with a validating signature) require actually triggering `release.yml` via a version tag, a real, visible, external action (creates a draft GitHub Release, runs all 4 signing pipelines including the Windows one that's still blocked on Max's Azure setup). Did not trigger this without checking first. If the next real test release still doesn't produce `latest.json`, the next thing to check is whether `tauri-action`'s default `retryAttempts` (0, meaning the "retry once" fallback path) is enough under real 4-way matrix contention — bump it explicitly in `release.yml` if so.
