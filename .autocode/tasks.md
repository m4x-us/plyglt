...
**Open, not yet root-caused:** what actually causes `mode` to stop being `"interrupt"` mid-session while the interrupt-limited queue is still active — the trigger for finding #2 above. The fix makes the failure mode safe (always recoverable now) but doesn't explain why it happens. Live circumstantial evidence strongly suggests the underlying wake/unlock/idle interrupt-firing mechanism itself DID work correctly at least once during this session (Max found the app already mid-quiz, on a card testing the word he'd deliberately gotten wrong, after leaving the VM locked/idle and returning — consistent with a successful background-triggered interrupt) — so Task #166's core Done-When (does an OS event trigger a review session) has real, live, positive evidence, even though the *specific* wake-vs-unlock-vs-idle trigger and a clean before/after notification-popup couldn't be cleanly isolated once the dead-end bug interrupted the test sequence. Logged as a follow-up investigation, not blocking this task's closure — see `.autocode/debt.md`.
