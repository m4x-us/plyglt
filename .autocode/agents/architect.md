...
## Task #169 first pass (2026-08-06, same session) — new modules to know about

`store/srsStore.ts`'s `commitSession` contract changed: `void` → returns the resulting `CardProgress`. Its only real production caller (`hooks/useStudySession.ts` → `app/study/page.tsx`) was updated in the same commit; `rateCard`/`rateCardAndSaveSession` (zero production callers, confirmed via grep before touching anything) were left untouched — not worth changing a dead code path's signature too. New modules: `lib/reviewEvent.ts` + `lib/conflictResolution.ts` (pure, no React/Zustand — the sync merge algorithm), `store/syncStore.ts` (new persisted store, `SYNC_VERSION` 1), `store/syncMigrations.ts` (split out of `store/migrations.ts` under the 400-line cap — that file is now at 3xx-something lines again, re-exports `SYNC_VERSION`/`migrateSyncStore` so it stays the single canonical import path), `lib/storeVersionGuard.ts` (the shared `assertNotFutureVersion`, extracted so the migrations split didn't require a circular import). `components/StudyEmptyQueue.tsx` extracted from `app/study/page.tsx` — that route was independently found to be 5 lines over the 150-line cap BEFORE this session's own 2-line addition; both issues fixed together. Task #169 itself is NOT complete — the live Supabase client, real auth screens, and end-to-end sync verification are blocked on Max provisioning real credentials (`docs/SYNC_CREDENTIALS_CHECKLIST.md`). Full detail in cto.md's Task Cycle Log entry for Task #169.
