...
- OPEN (found during Task #183 audit): AGENTS.md's Test Assertion Quality Gate documents `.toBeGreaterThan(0)` as a 4th banned pattern, but the mechanical grep gate only checks 3 of the 4 — 6 unjustified instances remain outside `tests/entitlement.test.ts` (which was fixed). Task #227 created to close this gap.
- OPEN (found during Task #183 audit, pre-existing, not caused by #183): `tests/importBackup.test.ts:35-41` "handles v0 backup... via migration chain" test name/comment describes behavior (`activeSession` field, migration chain) that doesn't exist in `lib/importBackup.ts` — harmless duplicate test wearing a misleading label. Not yet a task; logged to debt.md.
