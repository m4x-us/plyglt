...
16. **2026-08-02/03 — the full curriculum content track (item #14's "next open question") is now DONE, in the sense that matters most: every one of the 4 CEFR levels has BOTH unit-count/density completion AND at least one real adversarial content-quality audit + remediation round.** This closes out the content track opened by item #9/#12 above. Summary of what happened, in order:
    ...
    - **What's NOT done and should NOT be assumed done**: the platform track (Batch 15 Windows/Linux, Batch 16 Sync, Batch 17 Mobile) is untouched by this content push and remains exactly where item #15b/below left it — Batch 15 still needs Max's Azure signing setup + real hardware verification, Batch 16 still needs Max's sign-off on the Supabase+FCM recommendation in `docs/SYNC_ARCHITECTURE.md`, Batch 17 is still blocked on Batch 16.
