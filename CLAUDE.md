...
- `lib/basePackLoader.ts` — the storage/network load path for base packs (cache validation, sha256 verification, download, offline stale-cache re-verification, eviction-generation guard). Extracted from `lib/packLoader.ts` under the Rule 1 400-line cap (Task #378). Imported ONLY by `lib/packLoader.ts` — a mechanical test in `tests/packLoader.test.ts` enforces this boundary.
...
- `lib/generationGuard.ts` — shared snapshot/bump/isStale invalidation primitive (`createGenerationGuard()`) guarding in-flight async work against writing back state that a concurrent eviction cleared. Used by `lib/basePackLoader.ts`; `lib/specialtyPackLoader.ts`'s hand-rolled equivalent is a tracked migration.
