...
grep -rn "\.toBeDefined()\|\.toBeTruthy()\|\.not\.toBeNull()\|\.toBeGreaterThan(0)" . --include="*.test.*" --exclude-dir=node_modules --exclude-dir=.claude --exclude-dir=.next | grep -v "existence-check:" && echo "FAIL: existence-only assertions found without justification" && exit 1 || true
```
