...
describe("InterruptHandler", () => {
  ...
  describe("push-registration cleanup through the real component tree (round-16 audit)", () => {
    it("unregisters the push token when a Pro user's license downgrades to free — driven by InterruptHandler's own Pro-gate unmounting InterruptHandlerCore, not a bare hook unmount()", async () => {
      ...
      // Round-18 audit fix: a real registration happened this session, so the compare-
      // and-swap nonce (a crypto.randomUUID() generated by production code) is present.
      await vi.waitFor(() => expect(mockUnregisterPushToken).toHaveBeenCalledWith("user-1", "device-1", expect.any(String), undefined));
